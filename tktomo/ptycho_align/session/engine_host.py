...
# The verb bodies below annotate their inner callables with the context type.
...
class EngineHost(JobHost):
    ...
    def __init__(self, *, event_capacity: int = 5000) -> None:
        ...
        super().__init__(event_capacity=event_capacity, thread_name="ptycho-align-compute")

    # -- job hooks ---------------------------------------------------------------------

    def _refuse(self, kind: str) -> str | None:
        """Exclusive verbs are refused outright while a run is in flight."""
    ...
    def summary(self, since_iteration: int = 0) -> SessionSummary:
        with self._state_lock:
            ...
            current_job = None if self._current is None else self._current.handle.job_id
